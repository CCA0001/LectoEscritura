...
if (!$archivo) {
    header("Location: ../../controllers/controlador_escritura.php?error=Archivo no encontrado");
    exit();
}
...
if (isset($evaluacion['error'])) {
    ...
    header("Location: ../../controllers/controlador_escritura.php?error=$error_msg");
} else {
    $id_archivo_db = $archivo['ID'];
    

    $retroalimentacion_escapada = mysqli_real_escape_string($conexion, $evaluacion['retroalimentacion']);
    ...
    header("Location: ../../controllers/controlador_escritura.php?mensaje=Evaluacion completada. Puntaje: {$evaluacion['puntaje_promedio']}/10");
}
exit();
?>