...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['archivo_pdf'])) {
    ...
    if ($tipo != 'pdf' && $tipo != 'txt' && $tipo != 'docx') {
        $error = "Solo se permiten archivos PDF, TXT o DOCX.";
        header("Location: ../controllers/controlador_escritura.php?error=" . urlencode($error));
        exit();
    } 
    ...
    $carpeta_base = __DIR__ . "/../public/ejercicios";
    ...
    $url_guardar = "public/ejercicios/pdf_escritura/" . $nombre_unico;
    ...
    if (move_uploaded_file($archivo['tmp_name'], $ruta_fisica)) {
        ...
        if (mysqli_query($conexion, $sql)) {
            ...
            header("Location: ../controllers/controlador_escritura.php?mensaje=" . urlencode($mensaje));
            exit();
        } else {
    ...
    header("Location: ../controllers/controlador_escritura.php");
    exit();
}
?>